...
st.info("IMPORTANTE: incluye como filtro los movimientos relacionados con los fondos de previsión y jubilación, \
    ya que si no los incluyes no se importarán a la herramienta y no podrás llevar un control de ellos.")
...
st.write("Para ello con el ratón buscamos un texto con un enlace llamado __ver mas movimientos__ y lo pulsamos tantas veces\
    como necesitemos para mostrar el primer movimiento del mes.")
st.image("./assets/helps/ver_mas_movimientos.png")

st.write("Ahora que están en la pantalla todos los datos disponibles (ingresos y gastos del mes seleccionado)\
    lo siguiente es volver a la parte de arriba de la página y pulsar en el botón de la esquina superior derecha llamada __exportar__.")
st.image("./assets/helps/exportar.png")

st.write("Por último, seleccionamos la opción __lista de movimientos visualizados en pantalla__ y\
    pulsamos en __aceptar__ y se descargará un fichero .xlm")
